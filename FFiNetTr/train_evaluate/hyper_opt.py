...
SEARCH_SPACE = {
    ...
    'prediction_layers': hp.choice('prediction_layers', [1, 2, 3, 4]),
    'prediction_dropout': hp.choice('prediction_dropout', [0.1, 0.2, 0.3]),
    'prediction_hidden_dim': hp.choice('prediction_hidden_dim', [256, 512, 128]),
    'batch_size': hp.choice('batch_size', [32, 64, 128]), 
    'patience': hp.choice('patience', [100, 300, 500]),
    'lr': hp.choice('lr', [3e-4, 1e-3, 2e-3]),
}
...
def run_opt(num_iters, seed, data_path, model_class, train_args, higher_is_better=False):
    ...
    trials = Trials()
    fmin(objective, SEARCH_SPACE, algo=tpe.suggest, max_evals=num_iters, rstate=np.random.default_rng(seed), trials=trials)
