...
from FFiNet.train_evaluate.utils import EarlyStopping, evaluate_score, data2iter, multitask_mse, multitask_print, save_results, MESLoss_Weighted
...
def train(
    ...
    if train_args.writer:
        writer = {'epoch': [], 'train_loss': [], 'val_loss':[]}

    early_stopping = EarlyStopping(patience=train_args.patience, 
                                   path=train_args.model_save_path, 
                                   model_type=train_args.model_type, 
                                   tolerance=train_args.tolerance)
    model.to(train_args.device)
    if train_args.task == 'regression' and train_args.num_tasks == 1:
        criterion = nn.MSELoss()
    elif train_args.task == 'regression' and train_args.num_tasks > 1:
        criterion = multitask_mse()
    ...
    for epoch in range(train_args.num_epochs):
        ...
        if epoch % train_args.interval == 0:
            if train_args.writer:
                ...
                writer['train_loss'].append(loss_sum / num_examples)
                writer['val_loss'].append(val_metric['loss'].item())
            if train_args.num_tasks == 1 and train_args.task == 'regression':
                print(f'epoch:{epoch}, loss ={loss_sum / num_examples: .4f}, '
                    f'val loss = {val_metric["loss"]:.4f}, '
                    f'val {train_args.metrics} = {np.round(val_metric[train_args.metrics][0], decimals=4)}')
            elif train_args.num_tasks == 1 and train_args.task != 'regression':
                 print(f'epoch:{epoch}, loss ={loss_sum / num_examples: .4f}, '
                 f'val loss = {val_metric["loss"]:.4f}, '
    ...
    if train_args.writer:
        train_args.count += 1
        pd.DataFrame(writer).to_csv((train_args.results_dir + train_args.model_type + '_loss_epoch' + str(train_args.count)) + '.csv')
    ...
    val_metric = evaluate_score(model, eval_loader, train_args)
    if train_args.num_tasks == 1 and train_args.task == 'regression':
        print(f'test {train_args.metrics} = {np.round(test_metric[train_args.metrics][0], decimals=4)}, '
          f'val {train_args.metrics} = {np.round(val_metric[train_args.metrics][0], decimals=4)}')
    elif train_args.num_tasks == 1 and train_args.task != 'regression':
...
def evaluate(n, data_path, model_class, model_args, train_args):
    ...
    for i in range(n):
        ...
        model = model_class(
            feature_per_layer=[65 if train_args.task_name[0] == 'pdbbind' else 66] + [model_args['hidden_dim']] * model_args['hidden_layers'], 
